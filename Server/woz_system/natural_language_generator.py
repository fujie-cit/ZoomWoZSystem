...
class NaturalLanguageGenerator:
    ...
    def generate(self, nlg_command: NaturalLanguageGeneratorCommand):
        
        '''
        以下は旧ドキュメント

        command は nlg_command.query.command に対応．
        slot は nlg_command.query.slot に対応．
        input は nlg_command.dm_result に対応．
        ...
        slot = nlg_command.query.slot
        input = nlg_command.dm_result

        # repeat の場合だけ特別扱い
        if command == "repeat":
            command = nlg_command.dm_result["command"]
            input = nlg_command.dm_result["slot"]
